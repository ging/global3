  ...
  def default_home_toolbar_menu
    ...
    items << {:key => :messages,
      ...
      :items => [
        ...
        {:key => :message_trash, :name => image_tag("btn/message_trash.png")+t('message.trash'), :url => conversations_path(:box => :trash), :options => {:link =>{:remote=> false}}}
      ]}
    ...
    if SocialStream.activity_forms.include? :document
      items << {:key => :resources,
        ...
        :items => [
          {:key => :resources_conferences,:name => image_tag("btn/btn_conferences.png")+t('conference.title'),:url => events_path},
  ...
  def default_profile_toolbar_menu(subject = current_subject)
    ...
    if SocialStream.activity_forms.include? :document
      ...
      items << {:key => :resources,
        ...
        :options => {:link => {:id => "resources_menu"}},
        :items => [
          {:key => :resources_conferences,:name => image_tag("btn/btn_conferences.png")+t('conference.title'),:url => subject == current_subject ? events_path : polymorphic_path([subject,Event.new])},
